...
app.get("/filter", (req, res) => {
  ...
  const requestedItems = jokes.filter(
...
app.put("/jokes/:id", async (req, res) => {
  try {
    ...
    const searchIndex = jokes.findIndex((joke) => joke.id === updateID);
...
app.patch("/jokes/:id", async (req, res) => {
  try {
    const updateID = parseInt(req.params.id);
    const existingJoke = jokes.find((joke) => joke.id === updateID);
    const searchIndex = jokes.findIndex((joke) => joke.id === updateID);
...
app.delete("/jokes/:id", (req, res) => {
  try {
    ...
    if (deleteId > jokes.length) {
      res.send("Out of bound");
    } else {
      const searchIndex = jokes.findIndex((joke) => joke.id == deleteId);
